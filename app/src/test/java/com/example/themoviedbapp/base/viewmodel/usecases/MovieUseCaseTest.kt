...
class MovieUseCaseTest : TestCase() {
    ...
    fun `test getSpecificMovie by providing movieID`() = runBlocking {
        ...
        coEvery {
           ...
           ResultCallBack.Success(mockMovie)
        )
        ...
        val movieList = (movieResponse as ResultCallBack.Success).data
        MatcherAssert.assertThat(movieList.release_date, CoreMatchers.`is`("2013-02-13"))
        MatcherAssert.assertThat(movieList.popularity, CoreMatchers.`is`(2.334))
        ...
        MatcherAssert.assertThat(
            movieList.spoken_languages[0].name,
            CoreMatchers.`is`("English")
        )
        MatcherAssert.assertThat(movieList.genres[0].name, CoreMatchers.`is`("Drama"))
        MatcherAssert.assertThat(movieList.adult, CoreMatchers.`is`(false))
        MatcherAssert.assertThat(
